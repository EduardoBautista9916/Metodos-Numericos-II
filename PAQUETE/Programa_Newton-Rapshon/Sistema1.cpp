void iniciar1(float *x0, int iteraciones, float tolerancia);
float **jacobina(float *x);
float *m_FuncionX(float *x);
float *calculo_xn(float *x, float **j1x, float *fx);
float **jacobina_inversa(float **jx);
float cal_tolerancia(float *x0, float *x1);
void imprimir(int n, float *x, float *fx, float **jx, float **j1x, float tol);
...
void iniciar1(float *x0, int iteraciones, float tolerancia){
    ...
    for(int i = 0;i <= iteraciones;i++){
        fx = m_FuncionX(x0);
        jx = jacobina(x0);
        j1x = jacobina_inversa(jx);
        x1 = calculo_xn(x0,j1x,fx);
        imprimir(i, x0, fx, jx, j1x, tol);
        tol = cal_tolerancia(x0, x1);
        x0 = x1;
        if(tol<=tolerancia)
            break;
    }
...
float *m_FuncionX(float *x){
    float *fx = new float[grado];
    fx[0]=(x[0]*x[0])+(x[0]*x[1])-10;
    fx[1]=x[1]+(3*(x[0]*(x[1]*x[1])))-50;
    return fx;
}
...
void formato_float(float valor){
    if(valor>=0){
        if(valor<10){
            printf("%.11f", valor);
        }
...
void formato2_float(float valor){
    if(valor>=0){
        if(valor<10){
            printf("%.8f", valor);
        }
